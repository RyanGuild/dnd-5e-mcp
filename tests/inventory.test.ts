...
describe('Inventory Management', () => {
  ...
  describe('Item Database Access', () => {
    ...
    it('should find equipment by ID', () => {
      const rope = getEquipmentById('rope-hemp-50-feet');
  ...
  describe('Adding Items to Inventory', () => {
    ...
    it('should add multiple quantities of an item', () => {
      const arrow = getEquipmentById('arrow');
      if (arrow) {
        addItemToInventory(testInventory, arrow, 20, false);
    ...
    it('should stack identical items', () => {
      const arrow = getEquipmentById('arrow');
      if (arrow) {
        addItemToInventory(testInventory, arrow, 10, false);
        addItemToInventory(testInventory, arrow, 15, false);
  ...
  describe('Removing Items from Inventory', () => {
    beforeEach(() => {
      ...
      if (arrow) addItemToInventory(testInventory, arrow, 30, false);
    });
    ...
    it('should reduce quantity when removing partial amounts', () => {
      removeItemFromInventory(testInventory, 'arrow', 10);

      const arrowItem = testInventory.items.find(item => item.item.id === 'arrow');
      expect(arrowItem?.quantity).toBe(20);
    });

    it('should remove item completely when quantity reaches zero', () => {
      removeItemFromInventory(testInventory, 'arrow', 30);
    ...
    it('should throw error when trying to remove more than available', () => {
      expect(() => {
        removeItemFromInventory(testInventory, 'arrow', 50);
      }).toThrow();
    });
  ...
  describe('Weight Calculations', () => {
    ...
    it('should handle multiple quantities in weight calculation', () => {
      ...
      if (arrow) {
        ...
        expect(currentWeight).toBe(arrow.weight * 20);
      }
    });
  ...
  describe('Complex Inventory Scenarios', () => {
    it('should handle a full adventurer\'s inventory', () => {
      ...
      const items = [
        ...
        { id: 'arrow', quantity: 30, equipped: false },
        { id: 'rope-hemp-50-feet', quantity: 1, equipped: false },
      ];
